...
class ItemSplitterUI(MachinePanelUIProxy):
    ...
    def onListUpdated(self, event):
        ...
        self.settings_grid.SetDimensionAndCall(
            (1, cur),
            lambda: self.onGridUpdated(event.lis)
        )
    ...
    def createLabelSelector(self, x, y):
        ...
        window = self.AddElement("ItemSplitterUI.label_selector", "label_selector_window")
        window.SetLayer(80)
        stack = window["stack"]
        ...
        for i in range(24):
            e = stack.AddElement("ItemSplitterUI.label_index_selector", "selector%d" % i)
    ...
    def createItemSelector(self, x, y):
        ...
        self.item_selector_window["close_btn"].asButton().SetCallback(lambda _:self.closeItemSelector())
        _i0 = 0
        ...
        for i in range(36):
            ...
            if item is None or item.id in item_ids:
                continue
            e = stack.AddElement("ItemSplitterUI.item_index_selector", "selector%d" % _i0)
            xsize, ysize = e.GetSize()
