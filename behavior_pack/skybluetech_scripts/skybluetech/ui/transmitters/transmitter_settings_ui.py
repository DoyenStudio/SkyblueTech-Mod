...
def rand_rgb_by_index(index):
    ...
    return float(r // 256 // 256) / 256, float(r // 256 % 256) / 256, float(r % 256) / 256
...
class TransmitterSettingsUI(UScreenNode):
    ...
    def Create(self):
        ...
        self.prior_add_btn = self.GetElement(PRIOR_ADD_BTN_NODE).asButton().SetCallback(self.onAddPrior)
        self.prior_sub_btn = self.GetElement(PRIOR_SUB_BTN_NODE).asButton().SetCallback(self.onSubPrior)
    ...
    def updateStack(self):
        ...
        for i in range(24):
            e = self.stack.AddElement("TransmitterSettingsUI.index_selector", "index_selector%d" % i)
    ...
    def updatePriority(self, priority):
        ...
        TransmitterSetPriority(self.dim, self.x, self.y, self.z, self.ap_side, self.priority_value).send()
