...
from ....common.define.flags import DEACTIVE_FLAG_STRUCTURE_BLOCK_LACK
...
class FermenterUI(MachinePanelUIProxy):
    def OnCreate(self):
        ...
        self.expected_temperature_label = self.GetElement(
            EXPECTED_TEMPERATURE_LABEL_NODE
        ).asLabel()
    ...
    def WhenUpdated(self):
        ...
        self.expected_temperature_label.SetText(
            "控温 %.1f°C" % self.sync.expected_temperature
        )
        self.volume_bar.SetFullPos(
            ...
            UICtrlPosData(
                "parent",
                relative_value=0.5
                - self.sync.expected_water_max_volume / POOL_MAX_VOLUME,
            ),
        )
        ...
        self.pool_img.SetFullSize(
            "y", UICtrlPosData("parent", relative_value=self.sync.content_volume_pc)
        )
        ...
        if (
            self.sync.structure_lack_blocks
            and sstatus == DEACTIVE_FLAG_STRUCTURE_BLOCK_LACK
        ):
            fmt = "§l§4结构缺失必须组件：\n" + "\n".join(
                "§9"
