...
from ...common.machinery_def.thermoelectric_generator import COLD_BLOCKS, HOT_BLOCKS
from ...common.define.facing import NEIGHBOR_BLOCKS_ENUM
from ...common.define.id_enum.machinery import THERMOELECTRIC_GENERATOR as MACHINE_ID
from ...common.ui_sync.machinery.thermoelectric_generator import (
    ThermoelectricGeneratorUISync,
)
from ...client.ui.machinery.thermoelectric_generator import ThermoelectricGeneratorUI
...
class ThermoelectricGenerator(BasicGenerator, GUIControl):
    ...
    def OnNeighborChanged(self, event):
        ...
        self.cool_value -= COLD_BLOCKS.get(former_block, 0) - COLD_BLOCKS.get(
            to_block, 0
        )
        ...
        if data["heat_value"] is None:
            for offset_x, offset_y, offset_z in NEIGHBOR_BLOCKS_ENUM:
                block_name = GetBlockName(
                    self.dim, (self.x + offset_x, self.y + offset_y, self.z + offset_z)
                )
